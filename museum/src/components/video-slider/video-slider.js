...
pagination.addEventListener('click', (event) => {
  ...
  const bullets = pagination.querySelectorAll('.video-slider__bullets > .swiper-pagination-bullet');

  const currentIndex = [...bullets].findIndex(item => item.classList.contains('swiper-pagination-bullet-active'));

  if (!event.target.classList.contains('swiper-pagination-bullet') &&
      !event.target.classList.contains('video-slider__arrow')) return;
